...
frappe.query_reports["Custom General Ledger"] = {
	filters: [
		...
		on_change: function() {
				let branch_address = frappe.query_report.get_filter_value("branch_address");

				if (branch_address) {
					frappe.db.get_doc("Address", branch_address).then(doc => {
						let full_address = [
							doc.address_line1 || "",
							doc.address_line2 || "",
							doc.city || "",
						...
						frappe.query_report.set_filter_value("branch_address_list", full_address);
					});
				} else {
					frappe.query_report.set_filter_value("branch_address_list", "");
				}
			}
		},
		{
			fieldname: "branch_address_list",
			label: __("Branch Address List"),
			fieldtype: "Data",
		},
